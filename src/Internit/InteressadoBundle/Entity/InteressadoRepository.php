 ...
 */
class InteressadoRepository extends CrudRepository
{
    ...
    public function create(Interessado $cr)
    {
    ...
    public function allSubjects()
    {
    	$qb = $this->_em->createQueryBuilder()
    	->select('count(p)')
    	->from('InternitInteressadoBundle:InteressadoSubject', 'p')
    ...
    public function lastGroupSelected($subjectId)
    {
    	...
    	->from('InternitInteressadoBundle:Interessado', 'r')
    ...
    public function listAllGroupsThisSubject($subjectId)
    {
    	$qb = $this->_em->createQueryBuilder()
    	->select('g.nome', 'g.id')
    	->from('InternitInteressadoBundle:InteressadoSubject', 's')
    	->join('s.groups', 'g')
    	->where('s.id = :subject')
   		->orderBy('g.id', 'ASC')
    	->setParameter('subject', $subjectId);
    ...
    public function listEmailsThisGroup($group)
    {
    	$qb = $this->_em->createQueryBuilder()
    	->select('e.email')
    	->from('InternitInteressadoBundle:InteressadoGroup', 'g')
    	->join('g.emails', 'e')
    	->where('g.nome = :group')
